...
public class farmrunOverlay extends Overlay {
    ...
    private final AutoFarmRun plugin;

    @Inject
    private farmrunOverlay(Client client, AutoFarmRun plugin) {
        this.client = client;
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        this.panelComponent.getChildren().add(TitleComponent.builder().text("Scurrius Spine Redeemer").build());
        LineComponent timeout = buildLine("Started: ", String.valueOf(this.plugin.started));
        LineComponent state = buildLine("State: ", String.valueOf(this.plugin.getState()));
